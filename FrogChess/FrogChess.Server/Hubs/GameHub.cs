...
public async Task JoinGame(string gameId, string playerName)
{
    ...
    if (game == null)
    {
        ...
        if (reconnected != null)
        {
            ...
            string opponentName = (game.Green?.ConnectionId == Context.ConnectionId)
                ? game.Red?.Name ?? "Opponent"
                : game.Green?.Name ?? "Opponent";
            await Clients.Caller.SendAsync("TurnChanged", currentTurnColor, turnMessage, opponentName);
    ...
    await Clients.Client(game.Green!.ConnectionId!).SendAsync("GameStarted", GetBoardState(game), game.Red.Name);
    await Clients.Client(game.Red.ConnectionId!).SendAsync("GameStarted", GetBoardState(game), game.Green.Name);
...
public async Task Reconnect(string gameId, string playerName)
{
    ...
    private void SwitchTurn(Game game)
    {
        ...
        var greenOpponentName = game.Red?.Name ?? "Opponent";
        var redOpponentName = game.Green?.Name ?? "Opponent";
        _ = Clients.Client(game.Green!.ConnectionId!).SendAsync("TurnChanged", nextColor,
            $"{GetPlayerName(game, game.CurrentTurn)}'s turn", greenOpponentName);
        _ = Clients.Client(game.Red!.ConnectionId!).SendAsync("TurnChanged", nextColor,
            $"{GetPlayerName(game, game.CurrentTurn)}'s turn", redOpponentName);
    }
